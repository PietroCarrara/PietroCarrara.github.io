...
>
> - {{< spoiler label="First tip" >}}No one entered or left the room until the two of them left and noticed the missing key.{{< /spoiler >}}
> - {{< spoiler label="Second tip" >}}The keyring holds everything in a tight fit, so removing any key would make noise, alerting Alice.{{< /spoiler >}}
> - {{< spoiler label="Third tip" >}}During the search, the two of them were always together, and it was not possible to trick anyone.{{< /spoiler >}}
> - {{< spoiler label="Fourth tip" >}}James alone is the culprit.{{< /spoiler >}}
> - {{< spoiler label="Fifth and final tip" >}}The key Alice used to open the room at the shift start and the one they found in the room are different copies.{{< /spoiler >}}
>
>Answer: {{< spoiler label="Answer" >}}Before the shift starts, James removes the key for that room from his own keyring and hides it in the living room. In the room, James switches his keyring, which is missing a key, with Alice's, which has all.{{< /spoiler >}}
